...
public class ActivityDaoImpl extends ActivityDao {
    ...
    private static final String SQL_GET_ACTIVITY_BY_NAME =
    ...
    private static final String SQL_GET_ACTIVITIES_IN_LIMIT_BY_TYPE =
            "SELECT * from Activity order by type_id LIMIT ?, ?";

    private static final String SQL_GET_ACTIVITIES_IN_LIMIT_BY_NAME =
            "SELECT * from Activity order by name LIMIT ?, ?";

    private static final String SQL_GET_ACTIVITIES_IN_LIMIT_BY_NUMBER_OF_USERS =
            "select * from activity ac LEFT JOIN ActivityRequest req ON ac.activity_id=req.activity_id where (req.action='Add' AND req.status='Approved') OR req.activity_id IS NULL group by ac.activity_id ORDER BY count(req.activity_id) LIMIT ?, ?";
    @Override
    ...
    @Override
    public Activity find(String name) throws DaoException {
        transaction.init(this);
        Activity activity = null;
        try {
            preparedStatement = connection.prepareStatement(SQL_GET_ACTIVITY_BY_NAME);
    ...
    @Override
    public List<Activity> findInLimitByType(int size, int page) throws DaoException {
        transaction.init(this);
        List<Activity> listOfActivity = null;
        try {
            preparedStatement = connection.prepareStatement(SQL_GET_ACTIVITIES_IN_LIMIT_BY_TYPE);
    ...
    @Override
    public List<Activity> findInLimitByName(int size, int page) throws DaoException {
        transaction.init(this);
        List<Activity> listOfActivity = null;
        try {
            preparedStatement = connection.prepareStatement(SQL_GET_ACTIVITIES_IN_LIMIT_BY_NAME);
            preparedStatement.setInt(1, size * page);
            preparedStatement.setInt(2, size);
            ResultSet resultSet = preparedStatement.executeQuery();
            listOfActivity = fillArray(resultSet);
    ...
    private static final String SQL_GET_ACTIVITIES_WITHOUT_USERS = "SELECT ac.* FROM Activity ac " +
            "WHERE NOT EXISTS ( SELECT req.activity_id FROM ActivityRequest " +
            "WHERE ac.activity_id=req.activity_id)";

    @Override
    public List<Activity> findInLimitByNumberOfUsers(int size, int page) throws DaoException {
        transaction.init(this);
        List<Activity> listOfActivity = null;
        try {
            preparedStatement = connection.prepareStatement(SQL_GET_ACTIVITIES_IN_LIMIT_BY_NUMBER_OF_USERS);
            preparedStatement.setInt(1, size * page);
            preparedStatement.setInt(2, size);
            ResultSet resultSet = preparedStatement.executeQuery();
            listOfActivity = fillArray(resultSet);
    ...
    private List<Activity> fillArray(ResultSet resultSet) throws SQLException, DaoException {
        ...
        while(resultSet.next()){
            ...
            for(ActivityRequest activityRequest : listOfRequests){
                ...
                if(activityRequest.getStatus().equals("Approved") &&
                activityRequest.getAction().equals("Add"))
    ...
    public List<Activity> findInLimit(int size, int page) throws DaoException {
        return findInLimitByName(size, page);
    }
}
