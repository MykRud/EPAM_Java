...
class RejectRequestTest {
    ...
    @InjectMocks
    private RejectRequest rejectRequestController;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.initMocks(this);
        mvc = MockMvcBuilders.standaloneSetup(rejectRequestController).setRemoveSemicolonContent(false).build();
    }
    ...
    void rejectRequest() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.get("/admin/activityRequestReject?id=1"))
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:/admin/activitiesRequests"));

        verify(requestService, times(1)).getRequest(1);
        verify(requestService, times(1)).rejectRequest(request);
